...
```
```
Spring中的单例类似于DCL(几乎一样)也是synchronized k-v 保存在 concurrentHashMap中

Spring 的Controller默认是单例的 不要使用非晶态成员变量 否则会逻辑混乱 多个接口可能会操作一个变量
```
...
```
JDK  和 CGLIB 的区别
1 jdk 利用拦截器(拦截器必须实现InvocationHandler)加上反射机制
生成一个实现代理接口的匿名类
在调用方法前用InvokeHandler来处理.只能对实现了接口的类生成代理
2 cglib 利用ASM开源包 对代理对象的class文件加载 通过修改字节码生成子类来处理
主要是对指定的类生成一个子类,并覆盖其中的方法实现增强,但是因为采用的是继承,对
final类或方法是无法继承的
3 选择
 >如果实现了接口 默认jdk动态代理实现AOP
 >如果实现了接口也可强制使用CGLIB
 >如没实现接口 必须CGLIB Spring 会自动在JDK和CGLIB间转换
